...
// ── Admin portal aggregates (read-only) ────────────────────────────────────
...
export interface AdminStats {
  ...
  wallets: {
    ...
    /** Creators ∪ signers ∪ party wallets (normalized). */
    uniqueAll: number
  }
  signatures: {
    total: number
...
/** Read-only dashboard aggregates for the operator admin portal. */
      ...
      createdLast24h: Number(
        (
          db
            ...
            .get(now - dayMs) as { n: number } | undefined
        )?.n ?? 0,
      ),
      createdLast7d: Number(
        (
          db
            .prepare(`SELECT COUNT(*) AS n FROM documents WHERE created_at >= ?`)
            .get(now - 7 * dayMs) as { n: number } | undefined
        )?.n ?? 0,
      ),
    },
    wallets: {
...
// ── Support tickets (operator queue + contact form) ─────────────────────────
...
function mapMessageRow(row: {
  ...
  const kind =
    row.author_kind === 'operator' || row.author_kind === 'system' || row.author_kind === 'customer'
      ? row.author_kind
      : 'customer'
  return {
    id: row.id,
...
export function extractDocumentSlugFromText(...parts: Array<string | null | undefined>): string | null {
...
function nextPublicTicketId(): string {
  // Short human-readable id: VL- + 8 hex chars
...
export function createSupportTicket(input: {
  ...
  const insertTicket = db.prepare(`
    INSERT INTO support_tickets (
      id, public_id, name, email, subject, status, document_slug,
      created_at, updated_at, resolved_at
    ) VALUES (?, ?, ?, ?, ?, 'open', ?, ?, ?, NULL)
  `)
  const insertMessage = db.prepare(`
    INSERT INTO support_ticket_messages (
      id, ticket_id, author_kind, author_name, body, resend_message_id, created_at
...
export function getSupportTicketById(id: string): SupportTicketRecord | null {
  const row = db
    .prepare(
      `SELECT id, public_id, name, email, subject, status, document_slug,
              created_at, updated_at, resolved_at
...
export function getSupportTicketByPublicId(publicId: string): SupportTicketRecord | null {
  const row = db
    .prepare(
      `SELECT id, public_id, name, email, subject, status, document_slug,
              created_at, updated_at, resolved_at
       FROM support_tickets WHERE public_id = ?`,
    )
    .get(publicId.trim().toUpperCase()) as
    | {
        id: string
        public_id: string
        name: string
...
export function listSupportTicketMessages(ticketId: string): SupportTicketMessageRecord[] {
  const rows = db
    .prepare(
      `SELECT id, ticket_id, author_kind, author_name, body, resend_message_id, created_at
...
export function listSupportTickets(opts: {
  ...
  const rows = db
    .prepare(
      `SELECT t.id, t.public_id, t.name, t.email, t.subject, t.status, t.document_slug,
...
export function addSupportTicketMessage(input: {
  ...
  body: string
  resendMessageId?: string | null
  /** When operator replies, bump ticket toward waiting_customer unless already closed/resolved. */
  bumpStatus?: boolean
  ...
  if (input.bumpStatus !== false && input.authorKind === 'operator') {
    if (ticket.status === 'open' || ticket.status === 'in_progress') {
      nextStatus = 'waiting_customer'
    }
  }
  if (input.authorKind === 'customer' && ticket.status === 'waiting_customer') {
